...
const START_TIME = 45 * 60; // frames at 60fps
...
export class AmazingMazeGame {
    ...
    _buildLevel() {
        ...
        // Recursive-backtracker perfect maze. cell.walls = {N,E,S,W} true=wall present.
        const cells = [];
        ...
        // Trap holes on a handful of non-start/goal cells.
    ...
    _wallRects() {
        // Build AABB rects for every present wall segment (including outer border).
        const { cw, ch } = this._cellSize();
        const rects = [];
        const { cols, rows } = this;
        for (let y = 0; y < rows; y++) {
    ...
    render() {
        ...
        if (this.state === S_READY) {
            this._panel(`LEVEL ${this.level}`, 'TAP OR TILT TO START');
    ...
    _renderHud() {
        ...
        const barW = BOARD_W;
        r.rect(BOARD_X, y + 10, barW, 3, COLORS.ink);
        r.rect(BOARD_X, y + 10, barW * Math.max(0, this.timeLeft) / (45 * 60), 3, seconds < 10 ? COLORS.danger : COLORS.accent);
    }
